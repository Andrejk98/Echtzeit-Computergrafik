 ...
 */
function createPlane(name: string, options: {
    width?: number,
    height?: number,
 ...
 * @returns The circular plane geometry.
 */
 ...
 */
function createBox(name: string, options: {
    width?: number,
    height?: number,
    depth?: number,
    widthSegments?: number,
...
{
    // TODO: option to get a smooth box for skyboxes etc.
        ...
        rotation: Quat)
    {
        ...
        for (let iy = 0; iy <= pHeightSegs; iy++) {
            ...
            for (let ix = 0; ix <= pWidthSegs; ix++) {
                // position
                const position = new Vec3(ix * segmentWidth - halfWidth, y, halfDepth).rotateQuat(rotation);
                positions.push(position.x, position.y, position.z);

                // texture coordinates
                const uvs = new Vec2(ix / pWidthSegs, iy / pHeightSegs).applyMat3(textureXform);
    ...
    buildPlane(depth, height, width, depthSegs, heightSegs, Quat.fromAxisAngle(new Vec3(0, 1, 0), Math.PI / -2));  // +x
    buildPlane(depth, height, width, depthSegs, heightSegs, Quat.fromAxisAngle(new Vec3(0, 1, 0), Math.PI / 2));   // -x
    buildPlane(width, depth, height, widthSegs, depthSegs, Quat.fromAxisAngle(new Vec3(1, 0, 0), Math.PI / 2));    // +y
    buildPlane(width, depth, height, widthSegs, depthSegs, Quat.fromAxisAngle(new Vec3(1, 0, 0), Math.PI / -2));   // -y
    buildPlane(width, height, depth, widthSegs, heightSegs, Quat.fromAxisAngle(new Vec3(0, 1, 0), Math.PI));       // +z
    buildPlane(width, height, depth, widthSegs, heightSegs, Quat.fromAxisAngle(new Vec3(0, 1, 0), 0));             // -z
 ...
 * @returns A screen-filling quad geometry.
 */
 ...
 */
function createTorusKnot(name: string, options: {
    knotRadius?: number,
    tubeRadius?: number,
    tubeSegments?: number,
...
} = {})
{
    ...
    for (let tubeIdx = 0; tubeIdx <= tubularSegments; ++tubeIdx) {
        ...
        for (let radIdx = 0; radIdx <= radialSegments; ++radIdx) {
            ...
            vertex = vertex.sub(P1).normalize();
    ...
    for (const [index, line] of lines.entries()) {
        ...
        if (ignoredLines.has(type)) {
            ...
            for (let i = 1; i <= 3; i++) {
                ...
                const position: number = parseInt(face[0]) - 1;
                const texCoord: number = face.length > 1 && face[1] !== '' ? parseInt(face[1]) - 1 : position;
                const normal: number = face.length > 2 ? parseInt(face[2]) - 1 : position;
 ...
 */
function expandObj(objData: ObjData, geometry: Geometry): void
{
    let positions: number[] = geometry.positions;
    let texCoords: number[] = geometry.texCoords;
    let normals: number[] = geometry.normals;
    let indices: number[] = geometry.indices;
 ...
 */
function uniquifyVertices(geometry: Geometry): void
{
    ...
    for (let i = 0; i < indexCount; i++) {
        const index: number = geometry.indices[i];
        if (uniqueIndices.has(index)) {
            ...
            geometry.texCoords.push(
                geometry.texCoords[index * 2],
                geometry.texCoords[index * 2 + 1]);
            geometry.normals.push(
                geometry.normals[index * 3],
                geometry.normals[index * 3 + 1],
                geometry.normals[index * 3 + 2]);
            geometry.tangents.push(
                geometry.tangents[index * 3],
                geometry.tangents[index * 3 + 1],
                geometry.tangents[index * 3 + 2]);
        } else {
 ...
 */
function getBaricentricCoordinates(geometry: Geometry): Array<number>
{
    ...
    const result = new Array<number>(geometry.positions.length).fill(0);
...
function removeUnusedFaces(name: string, indices: Array<number>, positions: Array<number>): void
{
    ...
    //     // Edges of the face.
    //     v0 = Vec3.fromArray(positions, indices[i] * 3);
    //     e1 = Vec3.fromArray(positions, indices[i + 1] * 3).sub(v0);
    //     e2 = Vec3.fromArray(positions, indices[i + 2] * 3).sub(v0);
    ...
    tangents?: Array<number>): void
{
    ...
    const unusedIndices = new Array<number>();
 ...
 */
function computeNormals(positions: Array<number>, indices: Array<number>): Array<number>
{
    ...
    for (let i = 0; i < indices.length; i += 3) {
        ...
        // TODO: this seems wrong - I need to normalize the cross product before adding it
        normals[i0] += nx;
        normals[i0 + 1] += ny;
        normals[i0 + 2] += nz;
 ...
 */
function computeTangents(positions: Array<number>, texCoords: Array<number>, indices: Array<number>, normals?: Array<number>): Array<number>
{
