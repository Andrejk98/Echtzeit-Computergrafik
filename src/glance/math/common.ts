...
/**
 * Checks if the given number is a "save" float.
 * JavaScript uses 64-bit floating point numbers, which means that most of the
 * numbers cannot be represented exactly by a float on the GPU. There's nothing
 * we can do about that, but at least I want to print a warning should we
 * encounter numbers that is way outside the normal range of a 32-bit float.
 *
 * This became visible, when I tried to use `performance.timeOrigin + performance.now()`
 * as the value for a "time" uniform in a shader. This value is > 1'700'000'000'000
 * and whatever arrived in the shader was just garbage.
 */
export function isSafeFloat(value: number): boolean
{
    return Math.abs(value) <= 16_777_216; // 2^24
}