...
function grad(hash: number)
{
    ...
    const cube: Vec3 = new Vec3(hash, hash / 2, hash / 4).floor().mod(2).mul(2).sub(1);
    ...
    const rhomb: Vec3 = cube
        .clone()
        .mul(1 - type)
        .add(cube.cross(cuboct).add(cuboct).mul(type));

    // Expand it so that the new edges are the same length
    // as the existing ones
    const grad: Vec3 = cuboct.mul(1.22474487139).add(rhomb);

    // To make all gradients the same length, we only need to shorten the
    // second type of vector. We also put in the whole noise scale constant.
    // The compiler should reduce it into the existing floats. I think.
    grad.mul((1.0 - 0.042942436724648037 * type) * 3.5946317686139184);
...
{
    const b: Vec3 = X.floor();
    const i4: Vec4 = new Vec4(X.x - b.x, X.y - b.y, X.z - b.z, 2.5);

    // Pick between each pair of oppposite corners in the cube.
    const v1: Vec3 = b.add(Math.floor(i4.dot(Vec4.of(0.25))));
    const v2: Vec3 = b
        .add(Vec3.xAxis())
        .add(new Vec3(-1, 1, 1).mul(Math.floor(i4.dot(new Vec4(-0.25, 0.25, 0.25, 0.35)))));
    const v3: Vec3 = b
        .add(Vec3.yAxis())
        .add(new Vec3(1, -1, 1).mul(Math.floor(i4.dot(new Vec4(0.25, -0.25, 0.25, 0.35)))));
    const v4: Vec3 = b
    ...
    let hashes: Vec4 = permute(new Vec4(v1.x, v2.x, v3.x, v4.x).mod(289.0));
    permute(hashes.add(new Vec4(v1.y, v2.y, v3.y, v4.y)).mod(289.0));
    permute(hashes.add(new Vec4(v1.z, v2.z, v3.z, v4.z)).mod(289.0)).mod(48.0);

    // Gradient extrapolations & kernel function
    const d1: Vec3 = X.sub(v1);
    const d2: Vec3 = X.sub(v2);
    const d3: Vec3 = X.sub(v3);
    const d4: Vec3 = X.sub(v4);
    const a: Vec4 = Vec4.of(0.75)
        .sub(new Vec4(d1.dot(d1), d2.dot(d2), d3.dot(d3), d4.dot(d4)))
        .max(0.0);
    const aa: Vec4 = a.mul(a);
    const aaaa: Vec4 = aa.mul(aa);
...
function openSimplex2SHeightmap(position: Vec3): Vec4
{
    ...
    const result: Vec4 = openSimplex2SDerivativesPart(position.clone()).add(
        openSimplex2SDerivativesPart(position.add(144.5)),
    );
